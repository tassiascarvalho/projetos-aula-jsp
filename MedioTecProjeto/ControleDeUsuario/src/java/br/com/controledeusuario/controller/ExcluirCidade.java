...
public class ExcluirCidade extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        //Recuperar o ID do objeto
        int idCidade = Integer.parseInt(request.getParameter("idcidadeexcluir"));
        try{
            GenericDAO dao = new CidadeDAOImpl();//Add importação
            dao.excluir(idCidade);
            request.getRequestDispatcher("ListarCidade").forward(request, response);            
        }catch(Exception ex){
            System.out.println("Erro no Servlet ExcluirCidade: "+ex.getMessage());
            ex.printStackTrace();
        }
    }
